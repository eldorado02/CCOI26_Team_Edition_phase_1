...
def f_inv(t: int) -> int:
    ...
    # Try each of the three branches and pick the consistent one.
    # Branch: x not in a-z, not in A-Z  →  x = t ^ 0x5a
    x = t ^ 0x5a
    if not (0x41 <= x <= 0x5a) and not (0x61 <= x <= 0x7a):
        return x

    # Branch: x in A-Z  →  x = ((t ^ 0x5a) + 0xe0) % 256
    x = ((t ^ 0x5a) + 0xe0) % 256
    if 0x41 <= x <= 0x5a:
        return x

    # Branch: x in a-z  →  x = ((t ^ 0x5a) + 0x20) % 256
    x = ((t ^ 0x5a) + 0x20) % 256
    if 0x61 <= x <= 0x7a:
        return x
...
def solve() -> str:
    # Step 1: invert the per-byte transform
    reversed_input = bytes(f_inv(t) for t in TARGET)

    # Step 2: undo the string reversal
...
if __name__ == "__main__":
    flag = solve()
    print(flag)
    # Expected: CCOI26{m1dn1ght_r3g1stry_k33p3r}
